...
const [showDPRPayment, setShowDPRPayment] = useState(false);
  ...
  // ====================================================
  // DPR PROJECTION HORIZON — CUSTOMER SELECTS 1 TO 15 YEARS
  // ====================================================
  // Keep the original GoSubsidy DPR method: one customer-selected tenure
  // controls both the financial projection horizon and the repayment schedule.
  // There is intentionally NO separate Loan Repayment Period field.
  ...
  const getDPRProjectKey = (project) => {
  ...
  return JSON.stringify({
    ...
    projectCost: Number(source.projectCost || 0),
    loanTenure: Number(source.loanTenure || 0),
  });
};
...
const money = (value) => {
  ...
  const calculations = useMemo(() => {
    ...
    // passed to the single /dpr-preview renderer. The renderer only displays them.
    const calculatorInterestRate =
      (Number(project.interestRate) || 0) / 100 / 12;

    // Use the same validated customer-selected horizon everywhere.
    // This keeps EMI/interest aligned with the 1–15 year DPR horizon.
  ...
  const buildDPRPayload = () => ({
    // Keep the existing GoSubsidy project/calculation method intact.
    // At the API boundary, make the single selected horizon authoritative
    // so stale saved tenure values cannot produce a 15-year preview but
    // a different tenure in the final AI DPR.
    project: {
      ...project,
      loanTenure: projectionYears,
    },
    // Authoritative DPR horizon derived from the customer-selected horizon.
    projectionYears,
    capitalCost: {
  ...
  // by the single /dpr-preview route using DPRPreview.jsx.
...
try {
  const payload = buildDPRPayload();
    ...
    // FINAL HORIZON GUARD: the exact payload sent to the backend must carry
    // the same customer-selected horizon in both places used by the DPR flow.
    // This does not change the financial calculation method; it only prevents
    // a stale saved value from crossing the API boundary.
    payload.projectionYears = projectionYears;
    payload.project = {
      ...(payload.project || {}),
      loanTenure: projectionYears,
      projectionYears,
    ...
    console.log("📘 DPR Payload — FINAL HORIZON:", {
      projectionYears: payload.projectionYears,
      projectLoanTenure: payload.project?.loanTenure,
      projectProjectionYears: payload.project?.projectionYears,
      calculationYears: payload.calculations?.years?.length,
    });
    ...
    // AI generation uses the same single DPRPreview renderer in premium mode.
    // No second preview component is used.
              ...
              <div className="col-md-6">
                ...
                <div className="input-group input-group-lg">
                  ...
                  <select
                    ...
                    aria-label="DPR projection horizon"
                  >
                    {Array.from(
                      { length: 15 },
                      (_, index) => index + 1
                ...
                <small className="text-primary fw-semibold">
                  Customer chooses the DPR projection period from 1 to 15 years.
                  The selected period controls the DPR financial projections and
      ...
      case "cashflow":
        return (
          <FinancialTable
            ...
            rows={[
              { label: "Capital / Promoter Contribution", key: "promoterContribution" },
              { label: "Bank Term Loan / Sources", key: "termLoan" },
              { label: "Profit Before Tax (PBT)", key: "profitBeforeTax" },
              { label: "Depreciation", key: "depreciation" },
              { label: "Total Sources / Inflow", key: "totalSources" },
              { label: "Less: Fixed Assets / Capital Expenditure", key: "fixedAssets" },
              { label: "Less: Tax Payment", key: "estimatedTax" },
              { label: "Less: Principal Repayment of Term Loan", key: "repayment" },
              { label: "Total Applications / Outflow", key: "totalOutflow" },
              { label: "Net Inflow / (Out Flow)", key: "netCashFlow" },
      ...
      {showDPRPayment && (
        <PaymentModal
          ...
          onSuccess={(paymentResult) => {
            console.log("✅ Premium DPR payment successful — unlocking this project and starting AI generation.");
            try {
              localStorage.setItem(
                "gosubsidy_dpr_premium_access",
                JSON.stringify({
                  product: "DPR_PRO",
                  projectKey: getDPRProjectKey(project),
                  paymentStatus: "paid",
                  paymentReference:
              ...
              console.warn("Unable to persist Premium DPR entitlement:", error);
            }
