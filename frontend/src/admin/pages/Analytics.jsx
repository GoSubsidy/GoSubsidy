...
export default function Analytics() {
  ...
  const [locationAnalytics, setLocationAnalytics] =
    useState(EMPTY_ANALYTICS.location_analytics);
  ...
  // ==========================================================
  // LOAD VISITOR LOCATION ANALYTICS
  // ==========================================================
    ...
    async () => {
      try {
        ...
        if (!response.ok || !data?.success) {
          throw new Error(
            data?.message ||
              "Unable to load visitor locations."
          );
        }

        setLocationAnalytics({
          total_location_visits:
            Number(data.total_location_visits) || 0,
  ...
  return (
    <Layout>
      ...
      {/* ======================================================
          VISITOR LOCATIONS
      ====================================================== */}
      <section className="analytics-card analytics-location-card mb-4">
        <div className="analytics-card-header">
          <div>
            <span className="analytics-eyebrow">
              VISITOR LOCATION
            ...
            <h3>
              Where GoSubsidy Users Are Visiting From
            </h3>
            <p>
              Approximate city, state and country based on the visitor IP.
              Exact addresses are not collected.
            </p>
          </div>
          <FaMapMarkerAlt className="analytics-location-header-icon" />
        </div>

        {locationError && (
          <div className="analytics-location-error">
            <FaExclamationTriangle />
          ...
          <>
            <div className="location-summary-grid">
              <div className="location-summary-card">
                ...
                <strong>
                  {number(
                    locationAnalytics.total_location_visits
                  )}
                </strong>
              </div>
              <div className="location-summary-card">
                ...
                <strong>
                  {number(
                    locationAnalytics.countries.length
                  )}
                </strong>
              </div>
              <div className="location-summary-card">
                ...
                <strong>
                  {number(
                    locationAnalytics.states.length
                  )}
                </strong>
              </div>
              <div className="location-summary-card">
                ...
                <strong>
                  {number(
                    locationAnalytics.cities.length
                  )}
                </strong>
              </div>
            </div>
            ...
            <div className="row g-4 mt-1">
              ...
              <LocationTable
                title="Top Countries"
                rows={locationAnalytics.countries}
                columns={[
                  { key: "country", label: "Country" },
            ...
            <div className="location-recent-wrap">
              <div className="location-recent-header">
                <div>
                  <strong>Recent Visitor Locations</strong>
                  <span>Latest tracked location events</span>
                </div>
              </div>

              {locationAnalytics.recent_visits.length ? (
                <div className="table-responsive">
                  <table className="analytics-table location-table">
                    <thead>
                      <tr>
                        <th>Time</th>
                        <th>City</th>
                    ...
                    <tbody>
                      {locationAnalytics.recent_visits.map(
                        (row, index) => (
                          <tr key={`${row.created_at}-${index}`}>
                            <td>{formatLocationTime(row.created_at)}</td>
                            <td><strong>{row.city || "Unknown"}</strong></td>
                            <td>{row.state || "Unknown"}</td>
                            <td>{row.country || "Unknown"}</td>
                            <td><span className="location-page-pill">{row.page_path || "/"}</span></td>
                          </tr>
                        )
                      )}
                    </tbody>
                ...
                <EmptyState
                  ...
                  text="New portal visits will appear here after the location tracking update is installed."
                />
              )}
            </div>
          </>
  ...
  return (
    <div className="col-xl-4">
      <div className="location-mini-card">
        <div className="location-mini-header">
          <div>
            <strong>{title}</strong>
            <span>Highest visitor volume</span>
          </div>
          <FaMapMarkerAlt />
        </div>
        ...
        {!rows.length ? (
          ...
          <div className="table-responsive">
            <table className="analytics-table location-table location-mini-table">
              <thead>
                <tr>
                  {columns.map((column) => (
                ...
                {rows.slice(0, 8).map((row, index) => (
                  <tr key={`${title}-${index}`}>
                    {columns.map((column) => (
                      <td key={column.key}>
                        {column.key === "visits" ? (
                          <strong>{number(row[column.key])}</strong>
                        ) : (
