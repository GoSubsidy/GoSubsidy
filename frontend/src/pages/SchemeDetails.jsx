...
export default function SchemeDetails() {
  ...
  const fullDescription = valueToText(scheme.description);
  const description = getShortDescription(fullDescription, title) ||
  ...
  return (
    <main className="bg-light min-vh-100" style={{ paddingBottom: "60px" }}>
      <div className="container pt-3">
        <button type="button" className="btn btn-link text-decoration-none text-secondary px-0 mb-2" onClick={() => navigate(-1)}>
      ...
      <section className="container">
        <div className="card border-0 shadow-lg overflow-hidden rounded-4" style={{ background: "linear-gradient(135deg, #062b63 0%, #0a4f9c 55%, #087f73 100%)" }}>
          ...
          <div className="card-body p-3 p-lg-4 text-white">
            ...
            <h1 className="fw-bold mb-1" style={{ maxWidth: "950px" }}>{title}</h1>
            <p className="mb-3" style={{ maxWidth: "820px", lineHeight: 1.4, opacity: 0.94, fontSize: "0.92rem" }}>{description}</p>
      ...
      <section className="container mt-2">
        <div className="row g-2">
          ...
          <InfoCard
            icon={<FaUsers />}
            label="Beneficiary"
            value={getShortText(valueToText(scheme.beneficiary), 120) || "Not specified"}
          />
        ...
        <div className="row g-2">
          <div className="col-lg-8">
            <DetailSection icon={<FaCheckCircle />} title="Benefits / Financial Support" accent="success">
              {benefits.length > 0 ? <CompactValueList items={benefits} icon="bi-check-circle-fill" max={180} /> : <EmptyValue text="Benefit information is not available yet." />}
            </DetailSection>

            <DetailSection icon={<FaClipboardCheck />} title="Eligibility" accent="primary">
              {eligibilityEntries.length > 0 ? <CompactObjectList entries={eligibilityEntries} max={280} /> : <EmptyValue text="Eligibility information is not available yet." />}
            </DetailSection>
            ...
            <DetailSection icon={<FaExternalLinkAlt />} title="Application Process" accent="info">
              {applicationEntries.length > 0 ? <CompactObjectList entries={applicationEntries} max={220} /> : <EmptyValue text="Application process information is not available yet." />}
            </DetailSection>
          </div>
...
function DetailSection({ icon, title, accent = "primary", children }) {
  return (
    <section className="card border-0 shadow-sm rounded-4 mb-2">
      <div className="card-body p-2 p-md-3">
        <div className="d-flex align-items-center gap-2 mb-2">
          <div className={`rounded-3 bg-${accent}-subtle text-${accent} d-flex align-items-center justify-content-center flex-shrink-0`} style={{ width: "36px", height: "36px" }}>{icon}</div>
...
function CompactValueList({ items, icon, max = 180 }) {
  return (
    <div className="d-grid gap-1">
      {items.map((item, index) => (
        <div key={`value-${index}`} className="d-flex align-items-start gap-2 border rounded-3 px-2 py-2 bg-light">
          <i className={`bi ${icon} text-success mt-1`} aria-hidden="true" />
          <div className="flex-grow-1 small">
            <ValueRenderer value={getShortText(valueToText(item), max)} />
          </div>
        </div>
      ))}
    </div>
...
function CompactObjectList({ entries, max = 240 }) {
  return (
    <div className="d-grid gap-1">
      {entries.map(([key, value]) => (
        <div key={key} className="border rounded-3 px-2 py-2">
          <div className="small text-uppercase fw-bold text-primary mb-1">{formatKey(key)}</div>
          <div className="small text-secondary">
            <ValueRenderer value={getShortText(valueToText(value), max)} />
          </div>
        </div>
      ))}
    </div>
...
function SidebarRow({ label, value, last = false }) {
  return (
    <div className={`d-flex justify-content-between gap-2 py-2 ${last ? "" : "border-bottom"}`}>
...
function EmptyValue({ text }) {
  return <div className="alert alert-light border mb-0 py-2 small text-muted">{text}</div>;
}
...
function getShortDescription(value, title = "") {
  ...
  // Remove common scraped-page boilerplate that should never appear in the hero.
  const cleaned = raw
    .replace(/^(Press Release\s*:?\s*)+/i, "")
    .replace(/^(Press Information Bureau\s*)+/i, "")
    .replace(/Ministry of Micro,\s*Small(?:,\s*&|\s*&)\s*Medium Enterprises/gi, "")
    .replace(/\bPosted On:\s*[^.]+/gi, "")
    .replace(/\bFor More Information Visit\b.*$/i, "")
    .replace(/\s+/g, " ")
    .trim();

  const withoutTitle = title
    ? cleaned.replace(new RegExp(`^${escapeRegExp(title)}\\s*[:|\\-–—]?\\s*`, "i"), "").trim()
    : cleaned;

  // Prefer the first two meaningful sentences, then cap the visual length.
  ...
  return getShortText(short || withoutTitle, 240);
}

function getShortText(value, max = 220) {
  ...
  return `${(lastSpace > max * 0.65 ? cut.slice(0, lastSpace) : cut).trim()}…`;
}
