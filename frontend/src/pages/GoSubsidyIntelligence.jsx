...
import { trackVisit, trackSearch } from "../services/analytics";
...
export default function GoSubsidyIntelligence({
  ...
  useEffect(() => {
    // Record the page visit in Supabase. The analytics service
    // prevents duplicate visits during the same browser session.
    trackVisit("GoSubsidy Intelligence", "/").catch((error) => {
      console.warn("[GoSubsidy Analytics] Intelligence visit:", error);
    });
  }, []);
  ...
  const analyse = (text) => {
    ...
    // Record only an actual submitted analysis/search.
    // Analytics failures never block the Intelligence response.
    trackSearch(clean, 0, "/").catch((error) => {
      console.warn("[GoSubsidy Analytics] Intelligence search:", error);
    });
